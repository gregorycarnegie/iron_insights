...
pub fn compute_viz(
    ...
    let (hist, scatter, dots_hist, dots_scatter) = create_all_viz_data_batch(&filtered, &lift_type, config)?;
...
fn create_all_viz_data_batch(
    ...
    for i in 0..df.height() {
        if let (Some(bw), Some(raw), Some(dots), Some(s)) = (
            ...
            if bw.is_finite() && raw.is_finite() && dots.is_finite() && 
               bw > 0.0 && raw > 0.0 && dots > 0.0 {
                valid_bw.push(bw);
                valid_raw.push(raw);
