...
impl DataProcessor {
    ...
    pub fn load_and_preprocess_data(&self) -> PolarsResult<DataFrame> {
        ...
        if let Some(parquet_path) = self.find_parquet_file() {
            println!(
                "🚀 Found Parquet file: {:?} - Loading at maximum speed!",
        ...
        if let Some(csv_path) = self.find_csv_file() {
            println!(
                "📂 Found CSV file: {:?} - Converting to Parquet for future speed boosts",
    ...
    pub async fn check_and_update_data(
        &self,
    ...
    async fn fetch_latest_revision(
        &self,
        ...
        let li_selector =
            Selector::parse("li").map_err(|e| format!("CSS selector error: {:?}", e))?;
        ...
        for li_element in document.select(&li_selector) {
            ...
            if text.trim().starts_with("Revision:") {
                ...
                let link_selector =
                    Selector::parse("a").map_err(|e| format!("CSS selector error: {:?}", e))?;
    ...
    async fn download_and_extract_data(
        &self,
    ...
    fn load_parquet_data(&self, path: &std::path::Path) -> PolarsResult<DataFrame> {
        ...
        println!(
            "⚡ Loaded {} records from Parquet in {:?}",
            df.height(),
            start.elapsed()
        );
        Ok(df)
        ...
        for (lift_name, column_name) in [
            ...
            if let Ok(column) = df.column(column_name) {
                if let Ok(f32_series) = column.f32() {
                    ...
                    let finite_count = f32_series
                        .into_no_null_iter()
                        .filter(|&x| x.is_finite())
                        .count();
                    let positive_count = f32_series
                        .into_no_null_iter()
                        .filter(|&x| x.is_finite() && x > 0.0)
                        .count();
                    let reasonable_count = f32_series
                        .into_no_null_iter()
                        .filter(|&x| x.is_finite() && x > 0.0 && x < 1000.0)
                        .count();

                    println!(
                        "📊 {} DOTS: {} total, {} null, {} finite, {} positive, {} reasonable",
                        lift_name,
                        all_count,
                        null_count,
                        finite_count,
    ...
    fn apply_sampling(&self, df: DataFrame) -> PolarsResult<DataFrame> {
        if df.height() > self.sample_size {
            println!(
                "📊 Sampling {} records from {} total for performance",
                self.sample_size,
                df.height()
            );
    ...
    fn create_sample_data(&self) -> PolarsResult<DataFrame> {
        ...
        println!(
            "✅ Generated {} sample records with DOTS scoring",
            df.height()
        );
        Ok(df)
    }
    ...
    pub fn convert_csv_to_parquet(
        ...
        if let (Ok(csv_meta), Ok(parquet_meta)) =
            (std::fs::metadata(csv_path), std::fs::metadata(&output_path))
        {
            let compression_ratio =
                (1.0 - parquet_meta.len() as f64 / csv_meta.len() as f64) * 100.0;
            println!(
