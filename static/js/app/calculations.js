...
function calculateDOTS(liftKg, bodyweightKg, sex = null) {
    const sexValue = sex || window.currentSex;
...
function updateUserMetrics(bodyweight, userLift) {
    ...
    if (bodyweight && userLift && !isNaN(bodyweight) && !isNaN(userLift)) {
        try {
            ...
            if (window.calculate_dots_and_level_for_lift_with_gender_wasm && window.calculate_wilks_wasm && window.calculate_ipf_gl_points_wasm) {
                console.log('Using WASM calculations with lift type:', window.currentLiftType);

                const dotsResult = window.calculate_dots_and_level_for_lift_with_gender_wasm(userLift, bodyweight, isMale, window.currentLiftType);
