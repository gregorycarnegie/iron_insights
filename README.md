...
- `iron_insights_web/` - Leptos CSR frontend built with Trunk
  ...
  - `seo/`, `robots.txt`, and `sitemap.xml` for static, crawlable SEO/GEO landing pages (one answer-first page per tool); regenerated by the pipeline's `04_seo_geo` stage from the published data, audit with `python scripts/seo_audit.py`
...
- `04_seo_geo` reads that published data and regenerates the static SEO/GEO pages, `robots.txt` and `sitemap.xml` under `iron_insights_web/`, stamping real medians, percentiles and the dataset size and date.
