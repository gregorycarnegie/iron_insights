...
function setToggle(element, type) {
    ...
    if (type === 'sex') {
        window.currentSex = value;
        console.log('Updated currentSex to:', window.currentSex);
    } else if (type === 'lift') {
        window.currentLiftType = value;
...
function updateEquipment() {
    ...
    if (document.getElementById('equipment-raw').checked) window.currentEquipment.push("Raw");
    if (document.getElementById('equipment-wraps').checked) window.currentEquipment.push("Wraps");
    if (document.getElementById('equipment-single-ply').checked) window.currentEquipment.push("Single-ply");
    if (document.getElementById('equipment-multi-ply').checked) window.currentEquipment.push("Multi-ply");

    // Default to Raw if nothing selected
    if (window.currentEquipment.length === 0) {
        window.currentEquipment.push("Raw");
    }
...
function setupWeightClassFilter() {
    ...
    if (weightClassSelect) {
        weightClassSelect.addEventListener('change', function (e) {
            window.currentWeightClass = e.target.value;
...
function setupFederationFilter() {
    ...
    if (federationSelect) {
        federationSelect.addEventListener('change', function (e) {
            window.currentFederation = e.target.value;
