...
use crate::models::{HistogramData, ScatterData, StatsData, VisualizationResponse};
...
pub fn serialize_all_visualization_data(
    ...
    let raw_hist_batch = RecordBatch::try_new(
        ...
        vec![
            ...
            Arc::new(Float32Array::from(pad_to_length(
                &histogram_data.values,
                raw_hist_len,
                0.0,
            ))),
            Arc::new(UInt32Array::from(pad_to_length(
    ...
    let raw_scatter_batch = RecordBatch::try_new(
        ...
        vec![
            ...
            Arc::new(StringArray::from(pad_to_length(
                &scatter_data.sex,
                raw_scatter_len,
                String::new(),
            ))),
    ...
    let dots_hist_batch = RecordBatch::try_new(
        ...
        vec![
            ...
            Arc::new(Float32Array::from(pad_to_length(
                &dots_histogram_data.values,
                dots_hist_len,
                0.0,
            ))),
            Arc::new(UInt32Array::from(pad_to_length(
                &dots_histogram_data.counts,
                dots_hist_len,
                0u32,
            ))),
            Arc::new(Float32Array::from(pad_to_length(
                &dots_histogram_data.bins,
                dots_hist_len,
                0.0,
            ))),
...
pub fn deserialize_visualization_response_from_arrow(
    data: &[u8],
) -> Result<VisualizationResponse, Box<dyn std::error::Error + Send + Sync>> {
        ...
        match data_type {
            "raw_histogram" => {
                if let (Some(values), Some(counts), Some(bins)) = (
                    ...
                    if !histogram_data.values.is_empty() {
                        histogram_data.min_val = histogram_data
                            .values
                            .iter()
                            .fold(f32::INFINITY, |a, &b| a.min(b));
                        histogram_data.max_val = histogram_data
                            .values
                            .iter()
            ...
            "dots_histogram" => {
                if let (Some(values), Some(counts), Some(bins)) = (
                    ...
                    if !dots_histogram_data.values.is_empty() {
                        dots_histogram_data.min_val = dots_histogram_data
                            .values
                            .iter()
                            .fold(f32::INFINITY, |a, &b| a.min(b));
                        dots_histogram_data.max_val = dots_histogram_data
                            .values
                            .iter()
                            .fold(f32::NEG_INFINITY, |a, &b| a.max(b));
                    }
            ...
            "dots_scatter" => {
                if let (Some(x), Some(y), Some(sex)) = (
                    ...
                    dots_scatter_data.sex =
                        (0..sex.len()).map(|i| sex.value(i).to_string()).collect();
                }
            }
            _ => {}
        }
...
pub fn serialize_stats_to_arrow(
    stats: &StatsData,
) -> Result<Vec<u8>, Box<dyn std::error::Error + Send + Sync>> {
    let schema = Schema::new(vec![
