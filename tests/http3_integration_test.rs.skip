...
// Test dependencies
use iron_insights::{
    config::AppConfig, data::DataProcessor, http3_server::Http3Server, models::AppState,
};
...
async fn test_http3_client_connection() {
    ...
    match client_result {
        ...
        Err(e) => {
            println!(
                "⚠️  HTTP/3 client connection failed (expected on some systems): {}",
                e
            );
...
async fn create_http3_client_and_test() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let req = Request::builder().method(Method::GET).uri("/").body(())?;
...
async fn test_http3_endpoint(
    ...
    // Send request
    let req = Request::builder().method(Method::GET).uri(path).body(())?;
